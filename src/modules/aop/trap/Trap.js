...
import { apply, construct } from "../pointcuts/pointcuts";
...
import { isUndefined } from "../../object-handling/introspection";
    ...
    if (!map.has(property)) {
      ...
      const match = (advices) => (key) => {
        for (const { rule, advice } of advices) {
          ...
          if (matches) node[key].push({ rule, advice });
        }
      };
      ...
      [
        () => {
          ...
          if (isImplicit || pointcut instanceof FunctionCallPointcut) {
            if (isImplicit) pointcut = apply().before(pointcut);
            return true;
          }
          return false;
        },
      ...
      [
        () => {
          ...
          if (isImplicit || pointcut instanceof ObjectConstructionPointcut) {
            if (isImplicit) pointcut = construct().before(pointcut);
            return true;
          }
          return false;
        },
