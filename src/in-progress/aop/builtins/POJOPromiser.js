...
/**
 * Builtin function using AOPProxy's API to create a POJO promiser,
 ...
 * @param {Object} obj A POJO object where each property refers to a value to use as is or to
 * a promise resolving to the value to use for that property.
 * @param {Object} options Options.
 * @param {boolean} [options.mutateTargetOnSettledPromiseQueue] Whether or not to mutate the
 * original given "obj" POJO object with the resolved values or not (defaults to false,
...
export default function POJOPromiser(
  ...
  const rules = {
    ["/.?/"]: get().around(function () {
        ...
        const promise = queuedPromise.then((data) => {
          ...
          return new Promise((resolve) => {
            ...
            if (obj && mutateTargetOnSettledPromiseQueue) {
              // Assign resolved values to the original target, so that
              // properties which value is a promise will be replaced with the resolved value.
      ...
      // All alternations ensure that no matter what the value, a promise which must be resolved
      // is always given.
      target.then = noActionFunc;
