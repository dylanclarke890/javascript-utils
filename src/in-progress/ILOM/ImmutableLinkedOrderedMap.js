import { lazyObject } from "../../aop/builtins/lazyObject";
 ...
 * @param {Array} [initialItems] Initial array of items to add to the map (defaults to an empty
 * array). Each item of the map MUST be an object with the following shape:
 * {
 ...
 *  ...
 * }
 * Where "[keyPropName]" is the name of the key property of the item. In this case the map
 * will map the value "keyPropValue" (the key) to the whole item, which will be treated as
 * the value of the map value, e.g.:
 ...
 * The same item format is used for all mutation operations where an item or items may be given
 * as parameters, e.g. for "set", "replace", etc... .
 ...
 * This is the default mode. In this mode, multiple mutation operations can occur on the same
 * map, each leading to a different branch. Lookups may be slower, particularly if a lot of
 * mutation operations have happened, as the structural sharing code will need to determine
 * the right branch of the version tree given a map which tries to lookup its data for a
 * given key. E.g.:
 ...
 * used further). The advantage is that a map in this mode will have the fastest lookups,
 * as in this mode there isn't any version tree.
 * E.g.:
 * const map = new ImmutableLinkedOrderedMap({
...
function hydrateNew({ keyPropName, mode }) {
  /**
   * Each new map created from client code has a reference to a shared data structure called
   * heap map.
   * Beside the fancy name, it basically stores all the nodes holding the values of the map
   * linked together in order to maintain insertion order and maps all the keys which were ever
   * added to all the maps forked from the original one.
   ...
   * For multiway mode each depth additionally maps to a stack which associates map versions for that
   * depth in the order they have been added to the version tree for that depth.
...
class ImmutableLinkedOrderedMap {
   ...
   *
   * @param {Object|Array} items An object representing the item to set or an array of items to
   * set (same structure as for the the "initialItems" option when creating a new map from scratch).
   * @param {boolean} prependMissing If set to true and the item is not in the map, it will be prepended
...
const ImmutableLinkedOrderedMapForMode = {
  [ImmutableLinkedOrderedMapMode.SINGLE]: {
    ...
    makeImmutableLinkedOrderedMapNode:
      makeSingleModeImmutableLinkedOrderedMapNode,
  ...
  [ImmutableLinkedOrderedMapMode.MULTIWAY]: {
    ...
    makeImmutableLinkedOrderedMapNode:
      makeMultiwayModeImmutableLinkedOrderedMapNode,
  ...
  [ImmutableLinkedOrderedMapMode.LIGHTWEIGHT]: {
    ...
    makeImmutableLinkedOrderedMapNode:
      makeLightweightModeImmutableLinkedOrderedMapNode,
